...
import { ChevronLeft, ChevronRight, CheckCircle, Play, Lock, Loader2, PlayCircle, FileText, Eye, X, Video, HelpCircle } from 'lucide-react';
...
const LessonView = () => {
    ...
    const [cohortPaymentStatus, setCohortPaymentStatus] = useState('unpaid');
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (enrolledCohort && enrolledCohort.course) {
                        setCohortPaymentStatus(enrolledCohort.pivot.payment_status);
                        setPaymentModel(enrolledCohort.payment_model);
    ...
    // Logic: If payment is split-50 and only partial paid, lock after 50% of lessons
    const lessonThreshold = Math.ceil(allLessons.length / 2);
    const isLocked = paymentModel === 'split-50' && cohortPaymentStatus === 'partial' && currentIndex >= lessonThreshold;
    ...
    return (
        <div style={{ height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ flex: 1, overflowY: 'auto' }}>
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
                    ...
                    <div style={{
                        ...
                        {isLocked ? (
                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#020617', zIndex: 10 }}>
                                <div style={{ textAlign: 'center', maxWidth: '450px', padding: '3rem' }}>
                                    <div style={{ width: '80px', height: '80px', background: 'rgba(255,255,255,0.05)', borderRadius: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 2rem auto', border: '1px solid rgba(255,115,22,0.3)' }}>
                                        <Lock size={36} color="#f97316" />
                                    </div>
                                    <h3 style={{ color: 'white', fontSize: '1.5rem', fontWeight: 900, marginBottom: '1rem', letterSpacing: '-0.02em' }}>Phase 2 Content Locked</h3>
                                    <p style={{ color: '#94a3b8', fontSize: '1rem', lineHeight: 1.6, marginBottom: '2.5rem', fontWeight: 500 }}>
                                        To continue past this lesson, please complete your balance payment. Full access is granted upon verification of total tuition.
                                    </p>
                                    <Link to={`/student/courses/${courseId}?cohortId=${cohortId}`} style={{ display: 'inline-flex', background: '#f97316', color: 'white', textDecoration: 'none', padding: '1rem 2rem', borderRadius: '14px', fontWeight: 900, fontSize: '1rem', boxShadow: '0 10px 15px -3px rgba(249, 115, 22, 0.3)' }}>
                                            ...
                                            {!quizStarted ? (
                                                <div style={{ textAlign: 'center', margin: 'auto' }}>
                                                    ...
                                                    <p style={{ color: '#94a3b8', fontSize: '1.1rem', marginBottom: '2.5rem', maxWidth: '500px', margin: '0 auto 2.5rem' }}>
                                                        Complete this assessment to validate your mastery of the concepts covered in this module.
                                                    </p>
                                                ...
                                                <div style={{ textAlign: 'center', margin: 'auto' }}>
                                                    ...
                                                    <h2 style={{ color: 'white', fontWeight: 900, fontSize: '2rem', marginBottom: '1rem' }}>
                                                        {quizResult.passed ? 'Assessment Mastered' : 'Action Required: Knowledge Gap Detected'}
                                                    </h2>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '3.5rem' }}>
                        ...
                        <div>
                            <div className="glass-panel" style={{ padding: '2rem', background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: '24px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    ...
                                    <p style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600, marginTop: '0.5rem', textAlign: 'center' }}>
                                        {lesson.file_url ? 'Resource is synchronized with your curriculum player.' : 'Everything you need is in the video stream.'}
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            {/* Student Global Asset Preview Modal */}
            {
                previewAsset && (
                        ...
                        <div style={{ background: 'white', overflow: 'hidden', width: '100%', maxWidth: '1000px', maxHeight: '92vh', position: 'relative', borderRadius: '32px', display: 'flex', flexDirection: 'column', boxShadow: '0 40px 100px rgba(0,0,0,0.4)' }}>
                            <div style={{ padding: '1.5rem 2.5rem', borderBottom: '1.5px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fcfdfe' }}>
                                ...
                                <button
                                    ...
                                    style={{ width: '42px', height: '42px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', background: '#f1f5f9', border: 'none', color: '#64748b', cursor: 'pointer', transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' }}
                            ...
                            <div style={{ flex: 1, padding: '2.5rem', background: '#f8fafc', overflow: 'auto', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                {previewAsset.type === 'pdf' ? (
                                    <iframe
                                        ...
                                        style={{ width: '100%', height: '70vh', border: 'none', borderRadius: '16px', boxShadow: '0 25px 50px rgba(0,0,0,0.12)' }}
                                        title="PDF Review"
                                    />
                                    ...
                                    <video
                                        ...
                                        style={{ width: '100%', maxHeight: '70vh', borderRadius: '16px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}
                                        controlsList="nodownload"
                                        onContextMenu={(e: any) => e.preventDefault()}
                                    />
                                ) : (
                                    <img
                                        src={previewAsset.url}
                                        style={{ maxWidth: '100%', maxHeight: '72vh', objectFit: 'contain', borderRadius: '20px', boxShadow: '0 40px 80px rgba(0,0,0,0.2)', userSelect: 'none', pointerEvents: 'none' }}
                                        alt="Asset Review"
                                        onContextMenu={(e: any) => e.preventDefault()}
                                    />
                                )}
                            ...
                            <div style={{ padding: '1rem 2.5rem', background: '#fcfdfe', borderTop: '1.5px solid #f1f5f9', textAlign: 'center' }}>
                                <p style={{ margin: 0, fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>Protected by Prestige Intelligence Global Security</p>
                            </div>
                        </div>
                    </div>
                )
